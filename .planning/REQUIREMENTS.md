...
- [ ] **AUTH-09**: Account Mode users can back up their data to the backend as an opaque, client-encrypted blob the server cannot read, and restore it on another device — **narrowed 2026-08-12**: confirmed the backend will never do bidirectional user-data sync (settled, not just undecided); this requirement now covers only the encrypted-backup case, contingent on Tomris resolving the backend's still-open "encrypted blob vs. pure user-cloud export" decision (currently leaning against it) — tracked in Phase 8. **Partially addressed 2026-09-08 (Plan 08-01):** the client-side half is done and device-verified — `BackupArchiveCipher` (Argon2id + AES-256-GCM, all crypto work run via `compute()`) produces an opaque, client-encrypted archive the server cannot read, with a passphrase-based key that travels with the archive so any device holding both the archive and the passphrase can decrypt it. **Contract proposed 2026-09-08 (Plan 08-02):** `docs/backend-contracts/encrypted-backup-blob.md` specifies the push/pull API for Tomris to confirm or reject — this is a written proposal, not an implementation; the checkbox stays unchecked until it is actually implemented. **Client push/pull implemented 2026-09-09 (Plan 08-03), flag-gated off:** `BackupApiClient`/`BackupSyncNotifier`/`BackupSyncSection` implement push/pull against Plan 08-02's proposed contract, entirely behind `BackupSyncConfig.enabled = false` and gated to Account Mode — but this is client-side readiness, not a satisfied requirement: there is still no real backend endpoint, no real network round trip has been observed, and no real cross-device restore has happened. The clause "restore it on another device" (via the server) remains unmet until Tomris confirms the contract, the backend is actually implemented, the flag is flipped on, and at least one real round trip is observed against a live server — see Phase 8's carried-forward note in ROADMAP.md
...
| AUTH-09 | Phase 8 | Partially addressed (client-side encryption done, 08-01, 2026-09-08; backend contract proposed, 08-02, 2026-09-08; client push/pull implemented behind an off-by-default flag, 08-03, 2026-09-09; still unmet: real backend, real round trip, real cross-device restore, and Tomris's contract confirmation) |
