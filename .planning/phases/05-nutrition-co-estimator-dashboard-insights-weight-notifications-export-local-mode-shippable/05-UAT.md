...
updated: "2026-07-29T16:22:51.009Z"
---
...
severity: blocker
note: "App-level bug CONFIRMED FIXED with real-device evidence, commit 2285c60 -- see Gaps below. Ran a new integration_test (not a mock -- a real FlutterLocalNotificationsPlugin + real flutter_timezone platform channel) directly on the user's connected Tab S7 FE: tz.local correctly resolved to Europe/Berlin (+02:00, exactly matching the device), both scheduleMealReminder and scheduleWeighInReminder succeeded, and the OS's own pendingNotificationRequests() API confirmed 3 real pending notifications -- including one the user had scheduled themselves during their own retest, sitting unfired. **This shifts the remaining problem from 'app never schedules' to 'OS accepts the schedule but never delivers/displays it' -- almost certainly a device-level power-management restriction, not an app code bug.** Added debugPrint logging throughout the scheduling path so future diagnosis can watch adb logcat live. Next step for the user: on Samsung One UI (SM-T733/Tab S7 FE), standard Android 'battery optimization' is a SEPARATE setting from Samsung's own Device Care > Battery > Background usage limits (Sleeping apps / Deep sleeping apps / Never sleeping apps) -- please check whether this app is in one of Samsung's own restricted lists, since that's a common cause of exactly this symptom (schedule succeeds, delivery silently never happens) and isn't something app code can work around."
...
- truth: "Meal and weigh-in reminders actually fire as real OS notifications at their scheduled time"
  ...
  missing:
    - "OS-level delivery still doesn't happen even after the app-level fix is confirmed correct via real-device evidence (see fix_verification) -- most likely a Samsung-specific power-management restriction (Device Care > Battery > Background usage limits, separate from standard Android battery optimization), not an app code bug. User needs to check their device's Samsung-specific sleeping-apps list."
  fix_commit: "025bc55 (app-level timezone fix), 2285c60 (real-device verification + diagnostic logging)"
  fix_verification: "Reproduced first in a widget test deliberately isolated from the existing notification_service_test.dart (whose setUpAll pre-seeds tz.setLocalLocation in its own setUpAll, masking this exact gap for every other test in that file) -- test/domain/services/notification_service_timezone_test.dart confirmed scheduleMealReminder throws LateInitializationError against current code. Fixed by calling tz_data.initializeTimeZones() then resolving+setting the device's real IANA timezone via FlutterTimezone.getLocalTimezone(), with a graceful UTC fallback (via initializeTimeZones()'s own default) if the platform channel is unavailable (e.g. simulators). A second test confirms the real device-timezone path actually sets tz.local correctly, not just the fallback. Full suite (376 tests) green, flutter analyze clean.\n\nUser then did a genuinely fresh rebuild+restart of that fix and STILL saw no notifications fire, and explicitly asked for real device evidence rather than another theory. Built integration_test/notification_scheduling_test.dart -- a real (non-mocked) FlutterLocalNotificationsPlugin + real flutter_timezone platform channel, querying the OS's own pendingNotificationRequests() API directly -- and ran it on the user's actual connected device (Tab S7 FE, R52RB0FSSAX). Result: tz.local resolved to 'Europe/Berlin' with an offset that exactly matches the device's real UTC offset (+02:00 both), both scheduleMealReminder and scheduleWeighInReminder succeeded, and pendingNotificationRequests() confirmed 3 real OS-level pending notifications -- notably including one (#101, 'Lunch reminder') that this test run never scheduled itself, meaning it's a leftover from the user's own manual retest that the OS accepted and has been silently holding, unfired, ever since. This is definitive proof the app-level scheduling code is correct; the remaining gap is OS/device-level delivery, outside what app code controls."
  debug_session: ""
