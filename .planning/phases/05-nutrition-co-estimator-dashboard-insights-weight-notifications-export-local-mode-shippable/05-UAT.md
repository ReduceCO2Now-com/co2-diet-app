...
updated: "2026-08-03T12:37:34.230Z"
---
...
- truth: "Meal and weigh-in reminders actually fire as real OS notifications at their scheduled time"
  ...
  missing:
    - "Real-device confirmation that a notification actually fires at its exact scheduled time -- blocked on ONE remaining interactive step: granting the SCHEDULE_EXACT_ALARM special-access permission, which requires a human to tap through a system Settings screen (Android does not allow this to be granted via adb on this Knox-managed device -- pm/dumpsys package introspection for third-party packages is restricted at the OS/MDM level on this specific device, confirmed while investigating; system packages work fine via the same commands). Ask the user to enable a meal or weigh-in reminder in the app once more -- the just-in-time flow will now navigate to that Settings screen -- grant it, then this can be verified via adb shell dumpsys alarm (already confirmed working for reading alarm state on this device) without needing further pm/dumpsys package access."
